...
class UserSQLAlchemyRepository(AbstractClientUser):
    ...
    async def forgot_password(self, new_password:str, email:str):
        
        regex = r"[!@#$%^&*(),.?\":{}|<>]"
        validacion=re.search(regex, new_password)
        if validacion and len(new_password)>=8:
            ...
            raise HTTPException(status_code=400, detail="La contraseña no cumple con los requisitos minimos de seguridad")
